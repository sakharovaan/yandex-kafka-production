...
async def process_message(app: FastAPI, config: KafkaConfig, msg: ConsumerRecord):
    ...
    hdfs_file_path = f"data/message_{uuid.uuid4()}"
    with app.hadoop_client.write(hdfs_file_path, encoding="utf-8") as writer:
        writer.write(json.dumps(message) + "\n")
        logger.info(f"{message} written to HDFS as {hdfs_file_path}")
